...
class API {
  constructor(options = {}) {
    ...
    assert(`Invalid recordings directory provided. Expected string, received: "${typeof recordingsDir}".`, typeof recordingsDir === 'string');
...
function registerAPI(app, config) {
  config = objectSpread({}, DefaultConfig, config);
  ...
  router.get('/:recording', function (req, res) {
    ...
    const _api$getRecording = api.getRecording(recording),
          status = _api$getRecording.status,
  ...
  router.post('/:recording', bodyParser.json({
    limit: config.recordingSizeLimit
  }), function (req, res) {
    const recording = req.params.recording;

    const _api$saveRecording = api.saveRecording(recording, req.body),
          status = _api$saveRecording.status,
  ...
  router.delete('/:recording', function (req, res) {
    ...
    const _api$deleteRecording = api.deleteRecording(recording),
          status = _api$deleteRecording.status,
...
class Server {
  constructor(config = {}) {
    this.config = objectSpread({}, DefaultConfig, config);
    this.app = express();
    this.app.use(cors());
