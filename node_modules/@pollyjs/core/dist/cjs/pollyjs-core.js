...
$JSON && _export(_export.S + _export.F * (!USE_NATIVE$1 || _fails(function () {
  ...
  stringify: function stringify$$1(it) {
    var args = [it];
    var i = 1;
...
class Logger {
  ...
  connect() {
    this._middleware = this.polly.server.any().on('response', (...args) => this.logRequest(...args));
  }
  ...
  logRequest(request) {
    if (request.config.logging) {
      ...
      console.log(`${FORMATTED_ACTIONS[request.action]} ➞ ${request.method} ${request.url} ${request.response.statusCode} • ${request.responseTime}ms`, request);
    }
  }
...
class Container {
  ...
  register(Factory) {
    utils.assert(`Attempted to register ${Factory} but invalid factory provided. Expected function, received: "${typeof Factory}"`, typeof Factory === 'function');
    const type = Factory.type,
          name = Factory.name;
    utils.assert(`Invalid registration name provided. Expected string, received: "${typeof name}"`, typeof name === 'string');
    utils.assert(`Invalid registration type provided. Expected string, received: "${typeof type}"`, typeof type === 'string');
...
function HTTPHeaders(headers) {
  ...
  if (isObjectLike(headers)) {
    keys$2(headers).forEach(h => proxy[h] = headers[h]);
  }
...
function url(url, config = {}) {
  const parsedUrl = parseUrl(url, true); // Remove any url properties that have been disabled via the config

  keys$3(config).forEach(key => {
...
function headers(headers, config) {
  ...
  if (isObjectLike(config) && isArray$2(config.exclude)) {
    config.exclude.forEach(header => delete normalizedHeaders[header]);
  }

  return normalizedHeaders;
}
function body(body, config) {
...
function guidFor(str) {
  ...
  let slug = slugify(sanitize(str)); // Max the slug at 100 char
...
function validateRecordingName(name) {
  utils.assert(`Invalid recording name provided. Expected string, received: "${typeof name}".`, typeof name === 'string');
  utils.assert(`Invalid recording name provided. Received An empty or blank string.`, name.trim().length > 0);
}
function validateRequestConfig(config) {
  utils.assert(`Invalid config provided. Expected object, received: "${typeof config}".`, isObjectLike(config)); // The following options cannot be overridden on a per request basis

  ['mode', 'adapters', 'adapterOptions', 'persister', 'persisterOptions'].forEach(key => utils.assert(`Invalid configuration option provided. The "${key}" option cannot be overridden using the server configuration API.`, !(key in config)));
}

const freeze$2 = freeze$1;
const parse$1 = JSON.parse;
class HTTPBase {
  constructor() {
  ...
  send(data) {
    ...
    if (typeof body === 'string') {
      const contentType = this.getHeader('Content-Type'); // Write strings in utf-8
...
class PollyResponse extends HTTPBase {
  ...
  get statusText() {
    return utils.HTTP_STATUS_CODES[this.statusCode] || utils.HTTP_STATUS_CODES[DEFAULT_STATUS_CODE];
  }

  status(statusCode) {
    const status = _parseInt$2(statusCode, 10);

    utils.assert(`[Response] Invalid status code: ${status}`, status >= 100 && status < 600);
...
const EVENTS = symbol$1();

const EVENT_NAMES = symbol$1();

function assertEventName(eventName, eventNames) {
  utils.assert(`Invalid event name provided. Expected string, received: "${typeof eventName}".`, typeof eventName === 'string');
  utils.assert(`Invalid event name provided: "${eventName}". Possible events: ${[...eventNames].join(', ')}.`, eventNames.has(eventName));
}

function assertListener(listener) {
  utils.assert(`Invalid listener provided. Expected function, received: "${typeof listener}".`, typeof listener === 'function');
}
...
class EventEmitter {
  ...
  constructor({
    ...
    utils.assert('An array of supported events must be provided via the `eventNames` option.', eventNames && eventNames.length > 0);
  ...
  on(eventName, listener) {
    ...
    if (!events.has(eventName)) {
      events.set(eventName, new set$1());
    }
  ...
  emit(eventName, ...args) {
    ...
    return asyncToGenerator(function* () {
      assertEventName(eventName, _this[EVENT_NAMES]);

      if (_this.hasListeners(eventName)) {
        var _iteratorNormalCompletion = true;
        var _didIteratorError = false;
        var _iteratorError = undefined;

        try {
          for (var _iterator = getIterator$1(_this.listeners(eventName)), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  emitParallel(eventName, ...args) {
    var _this2 = this;

    return asyncToGenerator(function* () {
      assertEventName(eventName, _this2[EVENT_NAMES]);

      if (_this2.hasListeners(eventName)) {
        yield promise$1.all(_this2.listeners(eventName).map(listener => listener(...args)));
        return true;
      }
  ...
  emitSync(eventName, ...args) {
    ...
    if (this.hasListeners(eventName)) {
      this.listeners(eventName).forEach(listener => {
        ...
        utils.assert(`Attempted to emit a synchronous event "${eventName}" but an asynchronous listener was called.`, !(isObjectLike(returnValue) && typeof returnValue.then === 'function'));
      });
      return true;
    }
...
const ROUTE = symbol$1();

const POLLY = symbol$1();

const PARSED_URL = symbol$1();

const EVENT_EMITTER = symbol$1();

const SUPPORTED_EVENTS = ['identify'];
class PollyRequest extends HTTPBase {
  constructor(polly, request) {
    super();
    utils.assert('Url is required.', request.url);
    utils.assert('Method is required.', request.method && typeof request.method === 'string');
    ...
    this.requestArguments = freeze$3(request.requestArguments || []);
    this.promise = defer$1();
    this[POLLY] = polly;
    ...
    */
    ...
    this.action = null; // Lookup the associated route for this request

    this[ROUTE] = polly.server.lookup(this.method, this.url); // Handle config overrides defined by the route

    this._configure(this[ROUTE].config()); // Handle recording name override defined by the route
  ...
  get absoluteUrl() {
    const url$$1 = this.url;
    return isAbsoluteUrl(url$$1) ? url$$1 : new utils.URL(url$$1).href;
  }
  ...
  setup() {
    ...
    return asyncToGenerator(function* () {
      ...
      yield _this._emit('request'); // Setup the response

      _this.response = new PollyResponse();
      _this.didRespond = false; // Setup this request's identifiers, id, and order

      yield _this._identify(); // Timestamp the request

      _this.timestamp = utils.timestamp();
    })();
  }

  respond(status, headers$$1, body$$1) {
    var _this2 = this;

    return asyncToGenerator(function* () {
      utils.assert('Cannot respond to a request that already has a response.', !_this2.didRespond); // Timestamp the response

      _this2.response.timestamp = utils.timestamp(); // Set the status code and headers

      _this2.response.status(status).setHeaders(headers$$1); // Set the body without modifying any headers (instead of using .send())


      _this2.response.body = body$$1; // Trigger the `beforeResponse` event

      yield _this2._emit('beforeResponse', _this2.response); // End the response so it can no longer be modified

      _this2.response.end();

      _this2.responseTime = new Date(_this2.response.timestamp).getTime() - new Date(_this2.timestamp).getTime();
      _this2.didRespond = true;

      _this2.end(); // Trigger the `response` event


      yield _this2._emit('response', _this2.response);
    })();
  }
  ...
  _identify() {
    ...
    return asyncToGenerator(function* () {
      const polly = _this3[POLLY];
      const requests = polly._requests;
      const matchRequestsBy = _this3.config.matchRequestsBy;
      _this3.identifiers = {}; // Iterate through each normalizer

      keys$4(NormalizeRequest).forEach(key => {
        if (_this3[key] && matchRequestsBy[key]) {
          _this3.identifiers[key] = NormalizeRequest[key](_this3[key], matchRequestsBy[key]);
        }
      }); // Emit the `identify` event which adapters can use to serialize the request body

      yield _this3[EVENT_EMITTER].emit('identify', _this3); // Freeze the identifiers so they can no longer be modified

      freeze$3(_this3.identifiers); // Guid is a string representation of the identifiers

      _this3.id = md5(stringify(_this3.identifiers)); // Order is calculated on other requests with the same id
      // Only requests before this current one are taken into account.

      _this3.order = matchRequestsBy.order && !_this3.shouldPassthrough && !_this3.shouldIntercept ? requests.slice(0, requests.indexOf(_this3)).filter(r => r.id === _this3.id).length : 0;
    })();
  }
...
function invoke(_x, _x2, _x3) {
  return _invoke$1.apply(this, arguments);
}

function _invoke$1() {
  _invoke$1 = asyncToGenerator(function* (fn, route, req, ...args) {
...
function _emit() {
  _emit = asyncToGenerator(function* (route, eventName, ...args) {
    var _iteratorNormalCompletion4 = true;
    var _didIteratorError4 = false;
    var _iteratorError4 = undefined;

    try {
      for (var _iterator4 = getIterator$1(route.handlers), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
        const handler = _step4.value;
        ...
        try {
          for (var _iterator5 = getIterator$1(listeners), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
            const listener = _step5.value;
            yield invoke(listener, route, ...args);
          }
        } catch (err) {
...
class Route {
  ...
  constructor(recognizeResults, middleware) {
    ...
    if (result) {
      ...
      this.params = objectSpread({}, result.params);
  ...
  config() {
    return mergeConfigs(...this._orderedHandlers().map(handler => handler.get('config')));
  }
  /**
  ...
  intercept(req, res, interceptor) {
    ...
    return asyncToGenerator(function* () {
      ...
      try {
        for (var _iterator = getIterator$1(_this._orderedHandlers()), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          const handler = _step.value;

          if (handler.has('intercept') && interceptor.shouldIntercept) {
            yield invoke(handler.get('intercept'), _this, ..._arguments);
          }
        }
      } catch (err) {
        _didIteratorError = true;
  ...
  emit() {
    ...
    return asyncToGenerator(function* () {
      ...
      try {
        for (var _iterator2 = getIterator$1(_this2.middleware), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
          const m = _step2.value;
          yield emit(m, ..._arguments2);
        }
      } catch (err) {
  ...
  _valueFor(key) {
    ...
    try {
      for (var _iterator3 = getIterator$1(this._orderedHandlers()), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
        const handler = _step3.value;
...
class Handler extends map$1 {
  constructor() {
    super();
    this.set('config', {});
    this._eventEmitter = new EventEmitter({
      eventNames: ['request', 'beforeReplay', 'beforePersist', 'beforeResponse', 'response']
    });
  }
  ...
  intercept(fn) {
    utils.assert(`Invalid intercept handler provided. Expected function, received: "${typeof fn}".`, typeof fn === 'function');
...
class Middleware {
  constructor({
    ...
    this.paths.forEach(path => this._routeRecognizer.add([{
      path,
      handler: [handler]
    }]));
  }
...
const NAMESPACES = symbol$1();

const REGISTRY = symbol$1();

const MIDDLEWARE = symbol$1();

const HANDLERS = symbol$1();
...
function parseUrl$1(url) {
  const parsedUrl = new utils.URL(url);
  /*
...
class Server {
  ...
  host(path, callback) {
    ...
    utils.assert(`[Server] A host cannot be specified within another host.`, !host);
    this[HOST] = path;
    callback(this);
    this[HOST] = host;
  }
  ...
  timeout() {
    return utils.timeout(...arguments);
  }
  ...
  _lookupMiddleware(url) {
    const _parseUrl = parseUrl$1(url),
          host = _parseUrl.host,
          path = _parseUrl.path;
  ...
  _register(method, routes) {
    ...
    castArray(routes).forEach(route => {
      const _parseUrl2 = parseUrl$1(this._buildUrl(route)),
      ...
      if (router[HANDLERS].has(name)) {
        ...
        router.add([{
          path,
          handler: router[HANDLERS].get(name)
        }]);
      }
    });
    return handler;
  ...
  _registerMiddleware(routes) {
    ...
    castArray(routes).forEach(route => {
      ...
      */
      if ((!route || route === CHARS.STAR) && !this[HOST] && this[NAMESPACES].length === 0) {
        this[MIDDLEWARE].push(new Middleware({
          global: true,
          handler
        }));
      } else {
        const _parseUrl3 = parseUrl$1(this._buildUrl(route)),
  ...
  _recognize(method, url) {
    const _parseUrl4 = parseUrl$1(url),
  ...
  _buildUrl(path) {
    return utils.buildUrl(this[HOST], ...this[NAMESPACES], path);
  }
  /**
  ...
  _nameForPath(path = '') {
    const name = path.split(CHARS.SLASH).map(segment => {
...
const RECORDING_NAME = symbol$1();

const RECORDING_ID = symbol$1();

const PAUSED_MODE = symbol$1();

const FACTORY_REGISTRATION = new weakMap$1();
const EVENT_EMITTER$1 = new EventEmitter({
...
class Polly {
  ...
  set recordingName(name$$1) {
    validateRecordingName(name$$1);
    this[RECORDING_NAME] = name$$1;
    this[RECORDING_ID] = guidForRecording(name$$1);
  }
  /**
   * @readonly
   * @public
  ...
  set mode(mode) {
    const possibleModes = values$1(utils.MODES);

    utils.assert(`Invalid mode provided: "${mode}". Possible modes: ${possibleModes.join(', ')}.`, possibleModes.includes(mode));
  ...
  static register(Factory) {
    if (!FACTORY_REGISTRATION.has(Factory)) {
      FACTORY_REGISTRATION.set(Factory, container => container.register(Factory));
    }
  ...
  configure(config = {}) {
    ...
    utils.assert('Cannot call `configure` once requests have been handled.', this._requests.length === 0);
    utils.assert('Cannot call `configure` on an instance of Polly that is not running.', this.mode !== utils.MODES.STOPPED); // Disconnect from all current adapters before updating the config

    this.disconnect();
    this.config = mergeConfigs(DefaultConfig, this.config, config); // Register and connect to all specified adapters

    this.config.adapters.forEach(adapter => this.connectTo(adapter));
    /* Handle Persister */

    let persister = this.config.persister;
    ...
    if (persister) {
      ...
      utils.assert(`Persister matching the name \`${persister}\` was not registered.`, container.has(`persister:${persister}`));
  ...
  pause() {
    ...
    this.mode = utils.MODES.PASSTHROUGH;
  }
  /**
   * @public
   * @memberof Polly
  ...
  stop() {
    ...
    return asyncToGenerator(function* () {
      if (_this.mode !== utils.MODES.STOPPED) {
        _this.disconnect();

        _this.logger.disconnect();

        yield _this.persister && _this.persister.persist();
        _this.mode = utils.MODES.STOPPED;
        yield EVENT_EMITTER$1.emit('stop', _this);
      }
    })();
  }
  ...
  flush() {
    ...
    return asyncToGenerator(function* () {
      ...
      yield promise$1.all( // The NOOP is there to handle both a resolved and rejected promise
      // to ensure the promise resolves regardless of the outcome.
      _this2._requests.map(r => promise$1.resolve(r.promise).then(NOOP, NOOP)));
    })();
  }
  /**
  ...
  connectTo(nameOrFactory) {
    ...
    utils.assert(`Adapter matching the name \`${adapterName}\` was not registered.`, container.has(`adapter:${adapterName}`));
  ...
  disconnect() {
    ...
    try {
      for (var _iterator = getIterator$1(this.adapters.keys()), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        const adapterName = _step.value;
        this.disconnectFrom(adapterName);
      }
    } catch (err) {
      _didIteratorError = true;
...
const defineProperty$5 = defineProperty$3;
function beforeEach(_x, _x2, _x3) {
  return _beforeEach.apply(this, arguments);
}

function _beforeEach() {
  _beforeEach = asyncToGenerator(function* (context, recordingName, defaults) {
...
function afterEach(_x4, _x5) {
  return _afterEach.apply(this, arguments);
}

function _afterEach() {
  _afterEach = asyncToGenerator(function* (context, framework) {
    yield context.polly.stop();
    defineProperty$5(context, 'polly', {
      enumerable: true,
      configurable: true,

      get() {
        throw new Error(`[Polly] You are trying to access an instance of Polly that is no longer available.\n` + `See: https://netflix.github.io/pollyjs/#/test-frameworks/${framework}?id=test-hook-ordering`);
      }
...
function setupQunit(hooks, defaults = {}) {
...
function setupMocha(defaults = {}, ctx = global) {
