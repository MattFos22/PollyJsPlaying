...
import { ACTIONS, assert, timeout, MODES, URL, HTTP_STATUS_CODES, timestamp, buildUrl } from '@pollyjs/utils';
...
class Container {
  ...
  register(Factory) {
    assert(`Attempted to register ${Factory} but invalid factory provided. Expected function, received: "${typeof Factory}"`, typeof Factory === 'function');
    const type = Factory.type,
          name = Factory.name;
    assert(`Invalid registration name provided. Expected string, received: "${typeof name}"`, typeof name === 'string');
    assert(`Invalid registration type provided. Expected string, received: "${typeof type}"`, typeof type === 'string');
...
function validateRecordingName(name) {
  assert(`Invalid recording name provided. Expected string, received: "${typeof name}".`, typeof name === 'string');
...
function validateRequestConfig(config) {
  assert(`Invalid config provided. Expected object, received: "${typeof config}".`, isObjectLike(config)); // The following options cannot be overridden on a per request basis

  ['mode', 'adapters', 'adapterOptions', 'persister', 'persisterOptions'].forEach(key => assert(`Invalid configuration option provided. The "${key}" option cannot be overridden using the server configuration API.`, !(key in config)));
}
...
class PollyResponse extends HTTPBase {
  ...
  status(statusCode) {
    ...
    assert(`[Response] Invalid status code: ${status}`, status >= 100 && status < 600);
...
function assertEventName(eventName, eventNames) {
  assert(`Invalid event name provided. Expected string, received: "${typeof eventName}".`, typeof eventName === 'string');
  assert(`Invalid event name provided: "${eventName}". Possible events: ${[...eventNames].join(', ')}.`, eventNames.has(eventName));
}

function assertListener(listener) {
  assert(`Invalid listener provided. Expected function, received: "${typeof listener}".`, typeof listener === 'function');
}
...
class EventEmitter {
  ...
  constructor({
    ...
    assert('An array of supported events must be provided via the `eventNames` option.', eventNames && eventNames.length > 0);
  ...
  emitSync(eventName, ...args) {
    ...
    if (this.hasListeners(eventName)) {
      this.listeners(eventName).forEach(listener => {
        ...
        assert(`Attempted to emit a synchronous event "${eventName}" but an asynchronous listener was called.`, !(isObjectLike(returnValue) && typeof returnValue.then === 'function'));
      });
      return true;
    }
...
class PollyRequest extends HTTPBase {
  ...
  get absoluteUrl() {
    ...
    return isAbsoluteUrl(url$$1) ? url$$1 : new URL(url$$1).href;
  }
  ...
  respond(status, headers$$1, body$$1) {
    ...
    return asyncToGenerator(function* () {
      assert('Cannot respond to a request that already has a response.', !_this2.didRespond); // Timestamp the response

      _this2.response.timestamp = timestamp(); // Set the status code and headers
...
class Handler extends map$1 {
  ...
  intercept(fn) {
    assert(`Invalid intercept handler provided. Expected function, received: "${typeof fn}".`, typeof fn === 'function');
...
class Polly {
  ...
  set mode(mode) {
    const possibleModes = values$1(MODES);

    assert(`Invalid mode provided: "${mode}". Possible modes: ${possibleModes.join(', ')}.`, possibleModes.includes(mode));
  ...
  configure(config = {}) {
    ...
    if (persister) {
      ...
      assert(`Persister matching the name \`${persister}\` was not registered.`, container.has(`persister:${persister}`));
  ...
  connectTo(nameOrFactory) {
    ...
    assert(`Adapter matching the name \`${adapterName}\` was not registered.`, container.has(`adapter:${adapterName}`));
...
function generateRecordingName(assert$$1) {
  return assert$$1.test.testReport.fullName.join('/');
}
...
export { Polly, Timing, setupQunit, setupMocha };
