...
function timeout(time) {
  const ms = _parseInt$2(time, 10);

  return new promise$1(resolve => ms > 0 ? setTimeout(resolve, ms) : resolve());
}
...
function stringifyQuery(obj, options = {}) {
  return qs.stringify(obj, objectSpread({
    addQueryPrefix: true
  }, options));
}
/**
...
function arrayFormat(query) {
  const keys = (query || '').replace('?', '').split('&').map(str => decodeURIComponent(str.split('=')[0]));
...
class URL extends URLParse {
  ...
  toString() {
    return super.toString(obj => stringifyQuery(obj, {
      arrayFormat: this[ARRAY_FORMAT]
    }));
  }
...
function buildUrl(...paths) {
  const url = new URL(paths.map(p => p && (p + '').trim()) // Trim each string
...
function _serialize() {
  _serialize = asyncToGenerator(function* (body) {
    if (supportsBlob && body instanceof Blob) {
      return yield readBlob(body);
    }
...
function _serialize$1() {
  _serialize$1 = asyncToGenerator(function* (body) {
    if (supportsFormData && body instanceof FormData) {
      ...
      try {
        for (var _iterator = getIterator$1(body.entries()), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          const _step$value = slicedToArray(_step.value, 2),
                key = _step$value[0],
                value = _step$value[1];

          if (supportsBlob && value instanceof Blob) {
            const blobContent = yield readBlob(value);
...
function serialize$2(body) {
  if (supportsBuffer && body) {
    ...
    if (Buffer.isBuffer(body)) {
      buffer = body;
    } else if (isArray$1(body) && body.some(c => Buffer.isBuffer(c))) {
...
export { modes as MODES, actions as ACTIONS, httpStatusCodes as HTTP_STATUS_CODES, assert, timeout, timestamp, buildUrl, index as Serializers, URL };
