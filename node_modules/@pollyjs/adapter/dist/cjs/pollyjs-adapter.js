...
const ABORT = symbol$1();

const PASSTHROUGH = symbol$1();
...
class Adapter {
  ...
  shouldReRecord(pollyRequest, recordingEntry) {
    const config = pollyRequest.config;
  ...
  timeout(pollyRequest, {
    ...
    const timing = pollyRequest.config.timing;
  ...
  handleRequest(request) {
    ...
    return asyncToGenerator(function* () {
      const pollyRequest = _this.polly.registerRequest(request);

      pollyRequest.on('identify', (...args) => _this.onIdentifyRequest(...args));
      yield pollyRequest.setup();
      yield _this.onRequest(pollyRequest);

      try {
        const result = yield _this[REQUEST_HANDLER](pollyRequest);
        yield _this.onRequestFinished(pollyRequest, result);
        return result;
      } catch (error) {
        yield _this.onRequestFailed(pollyRequest, error);
        throw error;
      }
    })();
  }

  [REQUEST_HANDLER](pollyRequest) {
    var _this2 = this;

    return asyncToGenerator(function* () {
      const mode = _this2.polly.mode;
      let interceptor;

      if (pollyRequest.shouldIntercept) {
        interceptor = new Interceptor();
        const response = yield _this2.intercept(pollyRequest, interceptor);
      ...
      if (mode === utils.MODES.PASSTHROUGH || pollyRequest.shouldPassthrough || interceptor && interceptor.shouldPassthrough) {
        return _this2.passthrough(pollyRequest);
      }

      _this2.assert('A persister must be configured in order to record and replay requests.', !!_this2.persister);

      if (mode === utils.MODES.RECORD) {
        return _this2.record(pollyRequest);
      }

      if (mode === utils.MODES.REPLAY) {
        return _this2.replay(pollyRequest);
      } // This should never be reached. If it did, then something screwy happened.


      _this2.assert(`Unhandled request: ${pollyRequest.method} ${pollyRequest.url}.`, false);
    })();
  }
  ...
  passthrough(pollyRequest) {
    ...
    return asyncToGenerator(function* () {
      ...
      return _this3.onPassthrough(pollyRequest);
    })();
  }

  intercept(pollyRequest, interceptor) {
    var _this4 = this;

    return asyncToGenerator(function* () {
      pollyRequest.action = utils.ACTIONS.INTERCEPT;
      yield pollyRequest._intercept(interceptor);

      if (interceptor.shouldIntercept) {
        return _this4.onIntercept(pollyRequest, pollyRequest.response);
      }
    })();
  }
  ...
  record(pollyRequest) {
    ...
    return asyncToGenerator(function* () {
      ...
      return _this5.onRecord(pollyRequest);
    })();
  }
  ...
  replay(pollyRequest) {
    ...
    return asyncToGenerator(function* () {
      const config = pollyRequest.config;
      const recordingEntry = yield _this6.persister.findEntry(pollyRequest);

      if (recordingEntry) {
        yield pollyRequest._emit('beforeReplay', recordingEntry);

        if (_this6.shouldReRecord(pollyRequest, recordingEntry)) {
          return _this6.record(pollyRequest);
        }

        yield _this6.timeout(pollyRequest, recordingEntry);
        pollyRequest.action = utils.ACTIONS.REPLAY;
        const _recordingEntry$respo = recordingEntry.response,
              status = _recordingEntry$respo.status,
              statusText = _recordingEntry$respo.statusText,
              headers = _recordingEntry$respo.headers,
              content = _recordingEntry$respo.content;
        const normalizedResponse = {
          statusText,
          statusCode: status,
          headers: (headers || []).reduce((accum, {
            name,
            value
          }) => {
            accum[name] = value;
        ...
        return _this6.onReplay(pollyRequest, normalizedResponse, recordingEntry);
      }
      ...
      _this6.assert('Recording for the following request is not found and `recordIfMissing` is `false`.\n' + `${pollyRequest.method} ${pollyRequest.url}\n`, false);
    })();
  }
  ...
  onIdentifyRequest(pollyRequest) {
    return asyncToGenerator(function* () {
      const identifiers = pollyRequest.identifiers; // Serialize the request body so it can be properly hashed

      var _arr = ['blob', 'formData', 'buffer'];

      for (var _i = 0; _i < _arr.length; _i++) {
        const type = _arr[_i];
        identifiers.body = yield utils.Serializers[type](identifiers.body);
      }
    })();
  }
  /**
