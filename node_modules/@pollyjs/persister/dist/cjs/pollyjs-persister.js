...
var getByteLength = _interopDefault(require('utf8-byte-length'));
var setCookies = _interopDefault(require('set-cookie-parser'));
...
class Log {
  ...
  addEntries(entries = []) {
    ...
    [...entries, ...this.entries], (a, b) => a._id === b._id && a._order === b._order);
    this.sortEntries();
  }
...
function headersSize(request) {
  ...
  const headersString = request.method + request.url + keys.join() + values.join(); // startline: [method] [url] HTTP/1.1\r\n = 12
  // endline: \r\n = 2
  // every header + \r\n = * 2
...
class Request {
  constructor(request) {
    ...
    if (request.hasHeader('Content-Length')) {
      ...
      this.bodySize = this.postData && this.postData.text ? getByteLength(this.postData.text) : 0;
    }
  }
...
function headersSize$1(response) {
  ...
  const headersString = keys.join() + values.join(); // endline: \r\n = 2
  // every header + \r\n = * 2
  // add 2 for each combined header
...
class Response {
  constructor(response) {
    ...
    if (response.hasHeader('Content-Length')) {
      ...
      this.content.size = this.content.text ? getByteLength(this.content.text) : 0;
    }
...
function totalTime(timings = {}) {
  return keys$3(timings).reduce((total, k) => timings[k] > 0 ? total += timings[k] : total, 0);
}
...
class Persister {
  ...
  static get name() {
    utils.assert('Must override the static `name` getter.', false);
  }
  ...
  persist() {
    ...
    return asyncToGenerator(function* () {
      ...
      const creator = {
        name: CREATOR_NAME,
        version: _this.polly.constructor.VERSION,
        comment: `${_this.constructor.type}:${_this.constructor.name}`
      };
      ...
      try {
        for (var _iterator = getIterator$1(_this.pending), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          const _step$value = slicedToArray(_step.value, 2),
                recordingId = _step$value[0],
                _step$value$ = _step$value[1],
                name = _step$value$.name,
                requests = _step$value$.requests;

          const entries = [];
          const recording = yield _this.find(recordingId);
          let har;
          ...
          try {
            for (var _iterator2 = getIterator$1(requests), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
              const request = _step2.value;
              const entry = new Entry(request);

              _this.assert(`Cannot persist response for [${entry.request.method}] ${entry.request.url} because the status code was ${entry.response.status} and \`recordFailedRequests\` is \`false\``, request.response.ok || request.config.recordFailedRequests);
              /*
              ...
              */
              ...
              yield request._emit('beforePersist', entry);
              entries.push(entry);
            }
          } catch (err) {
          ...
          if (!_this.polly.config.persisterOptions.keepUnusedRequests) {
            _this._removeUnusedEntries(recordingId, har);
          }

          promises.push(_this.save(recordingId, har));
        }
      } catch (err) {
  ...
  recordRequest(pollyRequest) {
    ...
    const recordingId = pollyRequest.recordingId,
      ...
      if (!cache.has(recordingId)) {
        ...
        function () {
          var _ref = asyncToGenerator(function* () {
            const recording = yield _this2.findRecording(recordingId);

            if (recording) {
              _this2.assert(`Recording with id '${recordingId}' is invalid. Please delete the recording so a new one can be created.`, recording.log && recording.log.creator.name === CREATOR_NAME);
  ...
  save(recordingId) {
    ...
    return asyncToGenerator(function* () {
      yield _this3.saveRecording(..._arguments);

      _this3._cache.delete(recordingId);
    })();
  }
  ...
  delete(recordingId) {
    ...
    return asyncToGenerator(function* () {
      yield _this4.deleteRecording(..._arguments2);

      _this4._cache.delete(recordingId);
    })();
  }

  findEntry(pollyRequest) {
    var _this5 = this;

    return asyncToGenerator(function* () {
      const id = pollyRequest.id,
            order = pollyRequest.order,
            recordingId = pollyRequest.recordingId;
      const recording = yield _this5.find(recordingId);
      return recording && recording.log.entries.find(entry => entry._id === id && entry._order === order) || null;
    })();
  }
  ...
  assert(message, ...args) {
    utils.assert(`[${this.constructor.type}:${this.constructor.name}] ${message}`, ...args);
  }
  /**
  ...
  _removeUnusedEntries(recordingId, har) {
    const requests = this.polly._requests.filter(r => r.recordingId === recordingId && (r.action === utils.ACTIONS.RECORD || r.action === utils.ACTIONS.REPLAY));
