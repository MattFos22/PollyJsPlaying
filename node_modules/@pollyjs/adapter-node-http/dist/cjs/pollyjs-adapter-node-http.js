/**
* @pollyjs/adapter-node-http v1.4.2
*
...
var Adapter = _interopDefault(require('@pollyjs/adapter'));
...
function isBinaryBuffer(buffer) {
  ...
  const reconstructedBuffer = Buffer.from(utfEncodedBuffer, 'utf8'); // If the buffers are *not* equal then this is a "binary buffer"
...
function isContentEncoded(headers) {
  const contentEncoding = isObjectLike(headers) ? headers['content-encoding'] : '';
...
const nativeRequestMapping = new weakMap$1();
const keys$2 = keys$1;
class TransportWrapper {
  ...
  patch() {
    this.adapter.assert(`The ${this.name} transport has already been patched, please stop any running Polly instances`, !this.isPatched());
    const transport = this.transport;
    const request = transport.request,
          get = transport.get; // Save the native methods so we can restore them later on

    nativeRequestMapping.set(transport, {
      request,
      get
    });
    transport.request = this.createRequestWrapper(); // In Node 10+, http.get no longer references http.request by the export
  ...
  restore() {
    this.adapter.assert(`Cannot restore unpatched transport ${this.name}`, this.isPatched());
    const transport = this.transport;

    const _nativeRequestMapping = nativeRequestMapping.get(transport),
          request = _nativeRequestMapping.request,
          get = _nativeRequestMapping.get;
  ...
  getBodyFromChunks(chunks, headers) {
    const adapter = this.adapter; // If content-encoding is set in the header then the body/content
    ...
    if (isContentEncoded(headers)) {
      const hexChunks = chunks.map(chunk => {
        if (!Buffer.isBuffer(chunk)) {
          adapter.assert('content-encoded responses must all be binary buffers', typeof chunk === 'string');
          chunk = Buffer.from(chunk);
        }

        return chunk.toString('hex');
      });
      return stringify$1(hexChunks);
    }

    const buffer = mergeChunks(chunks); // The merged buffer can be one of two things:
  ...
  getChunksFromBody(body, headers) {
    ...
    const buffer = Buffer.from(body); // The body can be one of two things:
  ...
  passthrough(pollyRequest) {
    var _this = this;

    return asyncToGenerator(function* () {
      const transport = _this.transport;

      const _pollyRequest$request = slicedToArray(pollyRequest.requestArguments, 3),
            args = _pollyRequest$request[2];

      const method = pollyRequest.method,
            headers = pollyRequest.headers,
            chunks = pollyRequest.body;

      const _nativeRequestMapping2 = nativeRequestMapping.get(transport),
            nativeRequest = _nativeRequestMapping2.request;

      let _args = slicedToArray(args, 2),
          url = _args[0],
          options = _args[1];
      /**
      ...
      const request = nativeRequest.call(transport, objectSpread({}, options, {
        method,
        headers: objectSpread({}, headers)
      }, NodeUrl.parse(pollyRequest.url)));
      const requestPromise = new promise$1((resolve, reject) => {
      ...
      const responseBody = yield new promise$1((resolve, reject) => {
        const chunks = [];
        response.on('data', chunk => chunks.push(chunk));
        response.once('end', () => resolve(_this.getBodyFromChunks(chunks, response.headers)));
  ...
  respond(pollyRequest) {
    var _this2 = this;

    return asyncToGenerator(function* () {
      const pollyResponse = pollyRequest.response;

      const _pollyRequest$request2 = slicedToArray(pollyRequest.requestArguments, 2),
            req = _pollyRequest$request2[1];
      ...
      response.headers = objectSpread({}, pollyResponse.headers);
      response.rawHeaders = keys$2(response.headers).forEach(key => response.rawHeaders.push(key, response.headers[key]));
      yield new promise$1(resolve => process.nextTick(resolve));
      req.emit('response', response);

      const chunks = _this2.getChunksFromBody(pollyResponse.body, pollyResponse.headers);

      setImmediate$1(function emitChunk() {
        const chunk = chunks.shift();

        if (chunk) {
          response.push(chunk);

          setImmediate$1(emitChunk);
        } else {
          response.push(null);
        }
      });
  ...
  createRequestWrapper() {
    ...
    const _nativeRequestMapping3 = nativeRequestMapping.get(transport),
          nativeRequest = _nativeRequestMapping3.request;

    return function request(...args) {
      const req = nativeRequest.call(transport, ...args);
      const nativeWrite = req.write;
      const chunks = [];
      let ended = false; // Pause the opened socket so it doesn't read any of the incoming data

      req.once('socket', socket => socket.pause()); // Override req.write so we can save all the request body chunks
      ...
      req.end = (chunk, encoding, callback) => {
        ...
        const headers = typeof req.getHeaders === 'function' ? req.getHeaders() : req.headers || req._headers || {};
        const host = headers.host;

        const _host$split = host.split(':'),
              _host$split2 = slicedToArray(_host$split, 2),
              hostname = _host$split2[0],
              _host$split2$ = _host$split2[1],
              port = _host$split2$ === void 0 ? '80' : _host$split2$;

        const method = req.method,
              path = req.path;
        const parsedUrl = new utils.URL('');
...
class HttpWrapper {
  constructor({
    ...
    if (options.transports.includes('http')) {
      this.transports.push(new TransportWrapper(http, {
        name: 'http',
        adapter
      }));
    }

    if (options.transports.includes('https') && // Node 8 and below uses http.request under the hood for https.request
    ...
    semver.gte(process.version, '9.0.0')) {
      this.transports.push(new TransportWrapper(https, {
        name: 'https',
        adapter
      }));
    }
...
class HttpAdapter extends Adapter {
  ...
  onConnect() {
    ...
    this.assert('Invalid transports provided. At least one supported transport must be specified', transports.includes('http') || transports.includes('https'));
  ...
  onRecord(pollyRequest) {
    var _this = this;

    return asyncToGenerator(function* () {
      yield _this.passthroughRequest(pollyRequest);
      yield _this.persister.recordRequest(pollyRequest);
      yield _this.respond(pollyRequest);
    })();
  }

  onReplay(pollyRequest, {
    statusCode,
    headers,
    body
  }) {
    ...
    return asyncToGenerator(function* () {
      yield pollyRequest.respond(statusCode, headers, body);
      yield _this2.respond(pollyRequest);
    })();
  }

  onPassthrough(pollyRequest) {
    var _this3 = this;

    return asyncToGenerator(function* () {
      yield _this3.passthroughRequest(pollyRequest);
      yield _this3.respond(pollyRequest);
    })();
  }

  onIntercept(pollyRequest, {
    statusCode,
    headers,
    body
  }) {
    ...
    return asyncToGenerator(function* () {
      yield pollyRequest.respond(statusCode, headers, body);
      yield _this4.respond(pollyRequest);
    })();
  }

  passthroughRequest(pollyRequest) {
    return asyncToGenerator(function* () {
      const _pollyRequest$request = slicedToArray(pollyRequest.requestArguments, 1),
            transportWrapper = _pollyRequest$request[0];

      const res = yield transportWrapper.passthrough(pollyRequest);
      yield pollyRequest.respond(res.statusCode, res.headers, res.body);
    })();
  }

  respond(pollyRequest) {
    return asyncToGenerator(function* () {
      const _pollyRequest$request2 = slicedToArray(pollyRequest.requestArguments, 1),
            transportWrapper = _pollyRequest$request2[0];
